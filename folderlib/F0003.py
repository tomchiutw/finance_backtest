...
class F0001(ff.Folder):
    ...
    def F0001(self,time_index):
        ...
        if time_index in indicator_high_percent.data.index and not np.isnan(indicator_high_percent.data.loc[time_index,'High_Percent']) and \
            ...
            entry_condition5=last_spot_close<last_future_close
            entry_condition6=last_spot_close>last_future_close*(1+long_entry_percentage)
            entry_condition7=indicator_high_percent.data.loc[indicator_high_percent.data.index[indicator_high_percent_loc],'High_Percent']<short_entry_percentage
            ...
            if entry_condition1 and entry_condition2 and entry_condition3 and entry_condition4 and entry_condition5 :
                open_quantity=math.floor(leverage*account.balance/(commodity_future.contract_size*open_price+commodity_future.fee))
                if open_quantity!=0:
            ...
            close_condition2=indicator_settlement_dates.data.loc[time_index,'Days_To_Next_Settlement_Dates']<=settlement
