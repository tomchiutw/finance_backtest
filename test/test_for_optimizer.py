...
for year in range(start_year,end_year,n_steps):
    ...
    backtest_results[year]= f.folder_main(changable_var_dict,show_balance=False,show_details=True,show_pnl=False)
    # performance_summary
    ...
    # optimizer
    params = {
        'long_close_percentage': [0.02,0.04,0.08,0.10,0.15]
        # 'settlement':[2,5,10,15]
    }
    method='MAX_RETURN'
...
# save to pickle
gg.save_var_to_pickle(performance_summary,'F0001_VIXFUTURE','performance_summary')
gg.save_var_to_pickle(backtest_results,'F0001_VIXFUTURE','backtest_results')
gg.save_var_to_pickle(optimize_results,'F0001_VIXFUTURE','optimize_results')
# end all
