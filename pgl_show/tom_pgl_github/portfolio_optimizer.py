...
# =============================================================================
    ...
    def HIGH_MDD(self,idx,time_index):
        ...
        for col in strategy_list:
            col_index = self.template_data.columns.get_loc(col)
            performance_df[col]=gg.getPortfolio_Series(df=self.template_data,profitPercentage_Col=col,rf=0.02,translate=False)
...
# # =============================================================================
    ...
    def portfolio_backtest(self,backtest_start_date,backtest_end_date,start_balance=1000000,show_method=False,show_equityseries=False,show_details=False):
        ...
        if show_method and show_equityseries :
            gg.plot_all_columns_together(summary_df,bold_list=[self.method])
        if show_method and not show_equityseries:
            gg.plot_all_columns_together(summary_df,bold_list=[self.method],exception_list=[col for col in summary_df.columns if col!=self.method ])
        ...
        portfolio_backtest_results['performance']=gg.getPortfolio_Series(df=summary_df,profitPercentage_Col=self.method,rf=0.02)
