...
"""
import os
import yfinance as yf  # noqa: F401 (kept as original)
import pandas as pd
import generallib.general as gg  # noqa: F401 (kept as original)
import backtestlib.marketdata as bm
import get_base_dir as gbd


class Commodity:
        def __init__(self, name, symbol, category, currency, tick_size, contract_size, fee, data_source,
                     contract_list=['default'], exchange='', is_tradable=True, notes='', **kwargs):
             '''
        ...
        def init_settlement_dates(self):
            ...
            if self.category.lower() in margin_list:
                ...
                elif self.ignore_settlement_dates is True:
                    return
                elif self.ignore_settlement_dates is False:
...
class CommodityList:
    ...
    # UPDATED: fee=0, margin_info=0, ignore_settlement_dates stays True
    GOLD_FUTURE = {
                'name': 'GOLD_FUTURE',
                'symbol': 'GC=F',
                'category': 'future',
    ...
    # NEW: Commodity futures (all fee=0, margin=0, ignore_settlement_dates=True)
    WTI_CRUDE_OIL_FUTURE = {
                'name': 'WTI_CRUDE_OIL_FUTURE',
                'symbol': 'CL=F',
                'category': 'future',
                'currency': 'usd',
                'tick_size': 0.01,
    ...
    def list_commodities(cls, category_list=None, exception_list=None, transfer_to_commodity_type=True):
        if exception_list is None:
            exception_list = []
        commodities = [getattr(cls, attr) for attr in dir(cls)
                       if not attr.startswith("__") and not callable(getattr(cls, attr)) and attr not in exception_list]
        if category_list:
            filtered_commodities = []
            for category in category_list:
